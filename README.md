...
これで全ディレクトリ・全セッションで gate ツールと gate-loop スキルが使える。プロジェクト側に置くファイルはゼロ。ゲート運用にするリポジトリだけ、ルートに宣言ファイル `gate.yaml`(env / worksite / inert / checks / passline / cannot_see。全セクション任意)を置く。
...
README だけの変更に、確かめようがある証拠は無い。証拠の語彙(コンパイル・スクショ・操作記録…)に
文書の居場所が無いので、judge は「確認できず」しか返せず、その PR はレビュー可能化できないまま残る。
人間に押させて回避すると、ゲートは「守ると仕事が進まないもの」になり、いずれ全体が外される。

そこで `gate.yaml` に**動かしても出荷物の振る舞いが変わらない道筋**を宣言できる:
...
取り込みガードは `gh pr ready` のとき、デフォルトブランチとの分岐点から先端までの差分を数える。
**変わったファイルが全部 inert の中なら、提出の記録が無くても通す。1つでも外に出ていれば通常の照合に落ちる**
(そのファイル名を遮断の文言で名指しする)。

- **宣言を信じているのではない。** 通す/通さないを決めるのは宣言ではなく実際の差分で、
  判定はゲートがする。`inert` は「証拠が要らない」の申告ではなく、数える対象の宣言
- `*` は**ディレクトリ区切りを跨ぐ**(bash の `[[ == ]]` の意味論)。`docs/*` は `docs/a/b.md` に当たる
- 出荷物に混ざる道筋を入れてはいけない。ビルドの入力になっている JSON やアプリに同梱される
  文書は inert ではない(stock-radar の `docs/specs/03_design/product-kinds.json` がその例)
