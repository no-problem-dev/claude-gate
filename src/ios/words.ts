...
//   状態遷移。**git や gh のコマンドは実行しない**(ゲートは git・GitHub を読むだけで、書き込まない)。FSM の終着
...
// readiedAt / pushedAt は旧記録(提出が git・GitHub への書き込みを含んでいた頃)の時刻 —
...
export type ConceptCategory = "actor" | "record" | "vocabulary" | "derived" | "operation" | "external";
...
export const CONCEPT_CATEGORY_LABEL: Record<ConceptCategory, string> = {
  ...
  derived: "導出(保存しない)",
  operation: "操作",
  external: "ゲートの外(git・GitHub・作業環境)",
};
...
export const CONCEPTS = {
  ...
  // ゲートの外(git・GitHub・作業環境)
  branch: { ja: "作業ブランチ", en: "branch", category: "external" },
  draft_pr: { ja: "下書きPR", en: "draft PR", category: "external" },
  worksite: { ja: "作業場", en: "worksite", category: "external" },
...
export const DOMAIN_RELATIONS: DomainRelation[] = [
  ...
  { from: "awaiting_adoption", to: "submission", kind: "derives_from", label: "sha がまだデフォルトブランチに無い" },
  { from: "entered_default_branch", to: "submission", kind: "derives_from", label: "sha がデフォルトブランチの祖先" },
];
