...
Claude Code セッション(対話 / 並列ワーカー / cron)… N 個
 ...
 └── claude-gate デーモン(HTTP MCP・127.0.0.1:7350・マシンに1プロセス・launchd 常駐)
       ...
       ├─ subprocess: git / xcrun simctl / gate.yaml の checks コマンド(読み取りと証拠作りだけ。世界への書き込みコマンドは実行しない)
...
- **ゲートは世界を読むが、変えない**: ドメインの操作(MCP ツール・記録系 CLI)は世界への書き込みコマンド(git push・gh の書き込み)を一切実行しない。実行ファイルにそのコードが存在しない — 規約ではなく能力の不存在で守る。世界の読み取り(sha 解決・祖先関係・実物の照合)はゲートの本質なので自由(読み取りは何度やっても世界を変えない。べき等と決定論はこれで守られている)。世界を変えるのは消費者(人間・ガードを通ったエージェント)だけ
...
- **提出済みの先は状態ではなく導出**: 「デフォルトブランチに入った / 取り込み待ち」は世界のいまの姿なので保存しない。読み取りモデルが提出の記録の sha と origin のデフォルトブランチを毎回比較する
...
- submit は**記録だけ**であることをローカル bare リモートで固定する(提出しても bare の参照が一切動かない = 世界書き込みゼロのゴールデン)+ 全拒否経路 + べき等
