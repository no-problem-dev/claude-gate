...
- 「確認できず」はそのまま人間に渡す(reason と代わりの確認手段を添えて)。人間側の解決は `claude-gate confirm`(人間確認の記録 + 自動再判定)— 依頼はするが実行はしない
...
- 「確認できず」を OK・完了に言い換える
- 人間確認(`claude-gate confirm`)を自分で実行して「確認できず」を解消する(掃除 forget と同じく人間だけの操作)
