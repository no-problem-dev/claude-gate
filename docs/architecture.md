...
- **人間の動きは非同期 — 公式の遷移はローカルの状態に依存しない**: 検証は過去、人間の確認はその後、提出はさらに後で、そのときローカル(チェックアウト・worktree)は別の作業をしているのが正常(2026-07-22 の実例: 前日に検証した報告を、別ブランチで作業中に人間確認 → 提出した)。
  だから報告は**作業ブランチに属する**(open_report がオープン時に記録。旧報告はべき等な再オープンで補完される)。提出の三点照合は sourceSha = **ブランチ先端** = PR 先頭で、push もブランチ参照(`refs/heads/<branch>`)で行う — チェックアウトも worktree も未コミット変更の有無も関係ない。作業場(worksitePath)は証拠作り(ビルド・観測・run_check)とリポジトリ解決の道具。
  **ずれ**(検証したソースの後にブランチへ積まれたコミット)は状態ではなく**導出**: 注意と同じく保存せず、読み取りモデルが毎回 sourceSha とブランチ先端を比較して、発生した瞬間から報告カードに出す(提出の門で初めて発覚させない)。解消は 取り直し(推奨)か差分確認(人間の引き受け)の2つで、どちらも判定済みの記録を壊さない
...
合格 ─提出(sourceSha = ブランチ先端 = PR 先頭)→ 提出済み(終着。もう変わらない)
