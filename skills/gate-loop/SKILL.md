...
- ゲート本体を更新した直後(ツールの追加・引数の変更)は、MCP ツール一覧がセッション開始時に固定されるため **Claude Code セッションを開き直す**
...
open_report(
  ...
  behaviors: [
    { behavior: <動くと言っている動作(文で書く)>, check: <使う確かめ方(語彙から選ぶ)> },
    ...
  ]
)
...
- **動作は正直に書く**: 「〜できる」という操作の動作なら確かめ方は `interaction_log` / `ui_test` / `video`。静的スクショで覆いたいからと「〜が表示される」に宣言の側を弱めない(実際に起きた過ち)。状態の表示を確かめたいなら、最初からそう書く
