...
### 6. 共有は自由、提出は記録、取り込みに向かう操作はガード経由

**共有(feature ブランチへの `git push`・下書きPR の作成 `gh pr create --draft`)は自由領域** — ゲートを通さずいつでもできる(前提: デフォルトブランチは GitHub 側のブランチ保護で守られている)。

```
submit(worksitePath: <同上>, reportId: <合格した報告>)
→ 「検証したソース(sourceSha)を受け入れた」と記録する。記録だけの状態遷移 —
  git や gh のコマンドは実行しない。報告は終着(提出済み)になる
```

- 合格していない報告は提出できない(dirty 検証で sourceSha が無い報告も同じ)
- 提出済みの報告は終着。証拠の追加もできない — 続きは新しい作業名で開く
- 検証後にコミットが積まれていても提出の記録はできる(記録が指すのは検証したソース)。ただし取り込みに向かう操作はブランチ先端と記録の一致が要るので、先端を提出したいなら 取り直し(推奨)か、人間に差分確認を依頼して sourceSha を先端まで進めてから提出する

**提出のあと(PR 運用)**: `gh pr ready <ブランチ名>` を実行する — hook がデーモンに「ブランチ先端の sha に一致する提出済みの報告があるか」を照会し、一致すれば通る(一致しなければ遮断され、fix が返る)。PR 番号や URL 指定では先端を解決できないのでブランチ名(または対象ブランチをチェックアウトして引数なし)で実行する。

**取り込み(merge・デフォルトブランチへの push)は人間だけの操作**。エージェントは `gh pr merge` / デフォルトブランチへの直接 push / 非ドラフトの PR 作成を行わない(hook が遮断する)。main 直運用のリポジトリでは、提出の記録までがエージェントの仕事で、push は人間がダッシュボードの「取り込み待ち」を確かめて行う。
...
- テスト系の証拠は run_check(ゲートが実行)、判定は judge(ゲートが照合)、提出は submit(記録)に任せ、レビュー可能化は `gh pr ready` を hook のガード経由で行う
- 共有は自由に使う: feature ブランチへの push・下書きPR の作成はゲートを通さなくてよい
- 「確認できず」はそのまま人間に渡す(reason と代わりの確認手段を添えて)。人間側の解決は人間確認(ダッシュボードの記録フォーム / `claude-gate confirm`)
- 人間がセッション内で「確認した」と明言したときだけ、代筆として `claude-gate confirm` を実行してよい(note には人間が何をどう確認したかを書く。判断者は常に人間)
...
- 「確認できず」を OK・完了に言い換える
- 人間の明示の確認発話なしに人間確認(`claude-gate confirm` / POST /api/confirm)を実行して「確認できず」を解消する(判断は人間だけのもの。勝手な実行は監査ログに残る)
- rejected の後、原因を直さずに再試行だけ繰り返す
- ゲートを通さずに「見た感じ大丈夫」で済ませる
- 取り込みに向かう操作をガードの外で行う: `gh pr merge`・非ドラフトの PR 作成・デフォルトブランチへの直接 push はしない(取り込みは人間)。`gh pr ready` は submit で提出を記録してから — 記録なしの実行は hook が遮断する
